# 1. Min, Avg, Max Write a function get_min_avg_max (sequence) that returns a tuple with three values, the smallest,
...
def get_min_avg_max(sequence):
    result = min(sequence), sum(sequence) / len(sequence), max(sequence)
    return result
